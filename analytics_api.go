...
var (
	...
	ScheduleIndex 	map[string] bool
	PlaysIndex 			map[string] bool
)
...
func flushParquet(schema *arrow.Schema, b *array.RecordBuilder,
	...
	if err != nil {
		...
		if err != nil {
			...
			nbalake.PutFile(analyticsBucket, pf)
		}
...
func saveStandings() {
	...
	if err != nil {
		...
		nbalake.Put(analyticsBucket, fn, j)
	}
...
func parseBoxscores() []stats.NbaBoxscore {
	...
	boxscores := []stats.NbaBoxscore{}

	for k, _ := range ScheduleIndex {

		buf := nbalake.Get(rawBucket,
			fmt.Sprintf("%s%s", k, EXT_JSON))
